    ...
    <div class="w-100 bg-info text-white fixed-top">
            <div class="d-flex p-1 justify-content-around">
                <div class="d-flex justify-content-start">
                    ...
                    <a title="Moj profil" href="#" class="navbar-brand text-white ml-3 mt-2">{{session()->get('user')->first_name }} {{session()->get('user')->last_name}}</a>
                </div>
            <span  id="category" data-category="{{$data->category->id}}" class="h2 text-white mt-2 move-left align-content-center">{{$data->category->category_name}}</span>
            <a href="{{route('logout')}}" class="ml-5 btn btn-outline-dark h5 mt-2 text-center text-white">Odjavi se</a>
        </div>
