...
function sendAjaxRequestForTest() {
    ...
    $.ajax({
        ...
        error: function(r, s, e) {
				showModalAfterTestCompleted('Niste položili test! ' , 0, numberOfQuestions);
        }
});
}
function stopWatch() {
...
function printUsers(data){
    ...
    for(var item of data){
        ...
        if(item.is_blocked === 1)
            element+=`<span class="badge p-2 btn-warning text-white text-uppercase mr-3">B</span>`;
        if(item.active === 0)
           element+=`<span class="badge p-2 btn-secondary text-uppercase mr-3">N</span>`;
        element+=`
